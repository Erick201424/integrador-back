...
export class CreateEventUseCase {
    ...
    async execute(description: string,id_community: number,location: string,date:string,starts_at:string): Promise<Event | null> {
        ...
        try {
            const createEvent = await this.EventRepository.createEvent(description,id_community,location,date,starts_at);
            return createEvent;
        } catch (error) {
            return null;
        }
