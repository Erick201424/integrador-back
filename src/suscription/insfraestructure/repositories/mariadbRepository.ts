...
export class MariaDBRepository implements SuscriptionRepository {
    ...
    async getByEventId(id: number): Promise<Suscription[] | null> {
        try {
            const sql = "SELECT attendance.*, users.email AS user_email FROM attendance JOIN users ON attendance.id_user = users.id WHERE attendance.id_event = ?";
            const params: any[] = [id];
            console.log(params);
            const result = await query(sql, params);
